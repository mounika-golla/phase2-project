...
try {
String query = "SELECT * FROM railwaycrossingdetails";
...
try {
String query = "SELECT * FROM railwaycrossingdetails WHERE id = ?";
PreparedStatement statement = 
connection.prepareStatement(query);
...
try {
String query = "UPDATE railwaycrossingdetails SET name=?,address=?, landmark=?, train_schedule=?, person_in_charge=?, status=? WHERE id=?";
PreparedStatement statement = 
connection.prepareStatement(query);
...
String deleteFavoriteCrossingsQuery = "DELETE FROM favouritecrossing WHERE id = ?";
...
String deleteRailwayCrossingQuery = "DELETE FROM railwaycrossingdetails WHERE id = ?";
try (PreparedStatement deleteRailwayCrossingStatement = 
connection
...
try {
String query = "INSERT INTO railwaycrossingdetails (name, address, landmark, train_schedule, person_in_charge, status) VALUES (?, ?, ?, ?, ?,?)";
PreparedStatement statement = 
connection.prepareStatement(query);
statement.setString(1, crossing.getName());
statement.setString(2, crossing.getAddress());
...
String query = "SELECT rc.* FROM railwaycrossingdetails rc "
+ "JOIN favouritecrossing fc ON rc.id = fc.id";
PreparedStatement statement = 
connection.prepareStatement(query);
ResultSet resultSet = statement.executeQuery();
while (resultSet.next()) {
...
try (Connection connection = DBConnection.getConnection()) {
String sql = "INSERT INTO favouritecrossing (id) VALUES (?)";
PreparedStatement statement = 
...
try (Connection connection = DBConnection.getConnection()) {
String sql = "DELETE FROM favouritecrossing WHERE id = ?";
PreparedStatement statement = 
connection.prepareStatement(sql);
statement.setInt(1, crossingId);
statement.executeUpdate();
